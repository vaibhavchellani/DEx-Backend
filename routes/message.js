...
const web3 = new Web3(
  new Web3.providers.HttpProvider(
    "https://mainnet.infura.io/9c53Bai0EYh4fbE7elEE"
  )
);
...
function formOrder(req, res) {
  ...
  let request;
  try {
    request = JSON.parse(req.body.message);
  } catch (error) {
...
function getOrderParams(orderIn, availableVolume) {
  ...
  if (order.amount >= 0) {
    ...
    //difference
    order.availableVolumeBase = Math.abs(
      availableVolume
        .mul(order.price)
...
function getToken(token_address) {
  let result;
  const matchingTokens = config.tokens.filter(x => x.addr === token_address);
  ...
  if (matchingTokens.length > 0) {
    ...
    console.log(
      "dude it went into else , it wasnt supposed to , i know ! check line 202"
    );
  }
  return result;
}
